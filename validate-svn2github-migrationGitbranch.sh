...
##### checking if github have extra branches
  ...
  #git branch -a|grep -v '>'|grep -v main|awk -F / '{print $NF}'
  val=`for gi in $gbl
  do
    echo $svnb|grep $gi
  done`
  if [ $? -ne 0 ]
    then
      echo "$(tput setaf 2)***** VALIDATION FAILURE GITHUB repo has extra branches ***** $(tput setaf 7)"
      exit
    fi
  echo "Checking GITHUB Branches Verified SVN nd GITHUB Have same branches" 
  cd ..
