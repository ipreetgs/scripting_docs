...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  # Check out svn repo
 # svn checkout --username $SVN_USER --password $SVN_PASSWORD $SVN_REPO
  if [ $? -ne 0 ]
  then 
    echo "***** Validation FAILURE *****"
  ...
  for sbranch in $svnb
  do
    echo "checking svn $sbranch in git repo"
