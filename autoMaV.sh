...
if [ $? -ne 0 ]
 then
   echo "***** GITHUB REPO CREATION FAILURE *****"   
   exit
 fi
echo "repo created"
##STEP II
...
if [ $? -ne 0 ]
 then
   echo "***** POST REPO CREATION FAILURE *****"   
   exit
 fi
...
if [ $? -ne 0 ]
then
  echo "***** Disable repo FAILURE *****"
  ansible-playbook git_repo_set_perms.yml -e "git_mode=disable git_repo=$gitrepo"
fi
